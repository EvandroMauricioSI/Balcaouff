...
anuncio_model = anuncios_ns.model('Anuncio', {
    ...
    'condicao_produto': fields.String(required=True, description="Condição do produto"),
...
class AnuncioResource(Resource):
    ...
    @anuncios_ns.doc(
        description="Criação de um novo anúncio",
        body=anuncio_model  
    )
    ...
    def post(self):
        ...
        response = anuncios_controller.criar_anuncio(
            tipo, descricao, preco, foto, categoria, condicao_produto, avaliacao, anunciante, comprador, local
        )
